...
class Flight:
    ...
    async def turn(self) -> None:
        movement_command = await self.controller.read_movement_command()
        try:
            self.__send_move_command__(movement_command)
            await self.controller.display_movement(
                movement_command, self.drone.position, self.history.total_distance
            )

        except CrashException:
            await self.controller.display_crash_imminate(movement_command)

    async def play(self, max_round: int = 5) -> None:
        await self.controller.display_drone_start_position(
            self.drone.position, self.world
        )
        ...
        await self.controller.display_landing()
