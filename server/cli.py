...
from request_response import RequestMovementData
        ...
        while True:
            ...
            try:
                ...
                if direction in MOVEMENTS and distance.isdigit():
                    return tuple(axis * int(distance) for axis in MOVEMENTS[direction])
                else:
                    print("Invalid input. Please follow the format: DIRECTION DISTANCE")
            except ValueError:
                print("Invalid input. Please follow the format: DIRECTION DISTANCE")

    def display_crash_imminate(self, movement: Movement) -> None:
